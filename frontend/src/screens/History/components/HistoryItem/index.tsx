...
export const HistoryItem: React.FC<IHistoryItemProps> = ({
  ...
  const getImage = (path: string) => {
    return path ? path : imgPlaceholder;
  }

  return (
    <div className={styles.table_item}>
      {historyItem.fractionWatched === 1
        ? (
        ...
        : <img className={`${styles.collumn} ${styles.image}`} src={getImage(historyItem.lecture.previewImage)} alt="" />
      }
      <h4
      ...
      <div className={styles.collumn} style={{ color: '#FF8576' }}>{historyItem.tags.slice(0, 3).map(t => t + ' ')}</div>
